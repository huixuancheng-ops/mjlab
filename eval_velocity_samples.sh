#!/bin/bash
# Batch-evaluate velocity-walking policy .pt files using mjlab's built-in
# command-tracking metrics (error_vel_xy, error_vel_yaw, success_rate,
# mean_episode_length_s).
#
# Usage:
#   bash eval_velocity_samples.sh \
#     --sample-dir /path/to/checkpoints \
#     [--task Mjlab-Velocity-Flat-Unitree-G1] \
#     [--num-envs 1024] \
#     [--output-dir eval_results_velocity] \
#     [--gpu 0] \
#     [--pattern "*.pt"] \
#     [--max-samples 0]
...
usage() {
  echo "Usage: $0 --sample-dir DIR [options]"
  echo "  --sample-dir DIR       Directory containing checkpoint .pt files"
...
for ckpt in "${samples[@]}"; do
  ...
  echo "[eval] $name  $(date)"
  echo "========================================"
  uv run python -m mjlab.tasks.velocity.scripts.evaluate "$TASK" \
