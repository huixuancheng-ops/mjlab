"""Upload g1_tracking_dance1s2_hold_norm_sweep iter-4499 checkpoints to HuggingFace.

Companion to upload_dance1s2_hold_ckpts.py for the obs-normalization sweep
(run_dance_hold_norm.sh). Differences from upload_dance1s2_hold_ckpts.py:

  * Only the iter-4499 checkpoint per seed.
  * Uploads to a separate subfolder (default: analysis/dance_hold).
...
SOURCE = Path("logs/rsl_rl/g1_tracking_dance1s2_hold_norm_sweep")
REQUIRED_ITERS = (4499,)
...
def main():
  ...
  parser.add_argument(
    ...
    help=(
      ...
      "(default: logs/rsl_rl/g1_tracking_dance1s2_hold_norm_sweep)"
    ),
  )
  parser.add_argument(
    "--subfolder",
    default="analysis/dance_hold",
  ...
  for i in range(0, total, args.batch_size):
    ...
    commit_with_retry(
      ...
      batch,
      idx,
      f"Upload dance_hold norm ckpts batch {idx}/{n_batches}",
    )
    done += len(batch)
